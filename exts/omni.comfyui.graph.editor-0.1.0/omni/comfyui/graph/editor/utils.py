...
def get_service_prefix() -> str:
    """
    Return the prefix of where the service is mounted.

    Args:
        None

    Returns:
        str: URL prefix where service is mounted.

    """
    extension_name = get_extension_name()
    settings = carb.settings.get_settings()
    return settings.get_as_string(f"exts/{extension_name}/url_prefix")


def get_service_capture_path() -> str:
    """
    Return the URL path from where the captured images will be served from.

    Args:
        None

    Returns:
        str: URL path where captured images are available.

    """
    extension_name = get_extension_name()
    settings = carb.settings.get_settings()
    return settings.get_as_string(f"exts/{extension_name}/capture_path")
...
def get_captured_image_path() -> str:
    ...
    url_prefix = get_service_prefix()
    capture_path = get_service_capture_path()
...
def get_captured_image_directory() -> str:
    ...
    # FIXME: Make sure mount can still access temp directory with strange formatting
    carb.log_warn(f'Image temporarily stored at {captured_stage_images_directory}')
