...
namespace GlitchBlocksFX {

uniform float GlitchIntensity < ui_type = "drag";
> = .25;
uniform float GlitchScale < ui_type = "drag";
> = .80;
uniform float GlitchSpeed < ui_type = "drag";
> = 1.0;
uniform float GlitchWave < ui_type = "drag";
> = .50;
uniform float GlitchWaveSpeed < ui_type = "drag";
> = 1.40;
...
float4 PS(float4 vpos : SV_Position, float2 uv : TexCoord) : SV_Target {
  ...
  c = c1.a < c.a ? c1 : c;
  // c=c0;
  // c=exp2((frac(8*length(sin(floor(vp*133))))-.5)*6);
  // c=focus<.5?c1:c0;
  // c=c2.a>=c.a?c2:c;
  c.a = 1;
